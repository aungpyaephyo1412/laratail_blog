...
@endsection
@section('content')
    <div class="min-h-screen flex flex-col justify-center items-center pt-[75px]">
        <img class="h-96 max-w-full mb-5" src="{{@asset('storage/'.$blog->image)}}" alt="image description">
        <form action="{{ route('blog.update',$blog->id) }}" enctype="multipart/form-data" class="w-full" method="post">
            @csrf
            @method('PUT')
            <div class="mb-6">
